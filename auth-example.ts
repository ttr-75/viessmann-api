...
async function main() {
	...
	try {
		...
		console.log(`✅ ${installations.data.length} Installation(en) gefunden:\n`);
		let index = 0;
		...
		if (installations[0]?.gateways?.[0]?.devices?.[0]) {
			...
			const heatingFeatures: DeviceFeature[] = (features as DeviceFeature[])
				.filter(
					(f: DeviceFeature) =>
			...
			if (heatingFeatures.length > 0) {
				...
				heatingFeatures.forEach((feature) => {
					...
					console.log(
						`   ${feature.feature}: ${value?.value}${value?.unit || ""}`,
					);
				});
			}
		}
